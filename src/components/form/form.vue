  ...
  <section class="form-section">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item label="重复提醒" prop="delivery">
        <el-switch on-text="on" off-text="off" v-model="ruleForm.delivery" @change="changeOnOff"></el-switch>
      </el-form-item>
      <el-form-item label="重复时间" :required="redStar">
        <el-col :span="11">
          <el-form-item prop="starDate">
            <el-date-picker :disabled="forbidden" type="date" placeholder="选择开始日期" :picker-options="pickerOptionsStart" v-model="ruleForm.starDate" @change="starTimeChange" style="width: 100%;"></el-date-picker>
          </el-form-item>
        </el-col>
        <el-col class="line" :span="1">&nbsp;-</el-col>
        <el-col :span="12">
          <el-form-item prop="endDate">
            <el-date-picker :disabled="forbidden" type="date" placeholder="选择结束日期" :picker-options="pickerOptionsOver" v-model="ruleForm.endDate" @change="endTimeChange" style="width: 100%;"></el-date-picker>
          </el-form-item>
        </el-col>
      </el-form-item>
...
export default {
  data() {
    return {
      ...
      pickerOptionsStart: {
        disabledDate(time) {
          return time.getTime() < 1546272000000 || time.getTime() >= Date.now();
        }
      },
      pickerOptionsOver: {
        disabledDate(time) {
      ...
      rules: {
        ...
        type: [
          { required: true, message: '请选择代班事项类型', trigger: 'change' }
        ],
        way: [
  ...
  methods: {
    ...
    endTimeChange(val) {
      ...
      this.pickerOptionsStart = {
        disabledDate(time) {
          return time.getTime() < 1546272000000 || time.getTime() >= endTime;
        }
      }
    },
    changeOnOff(val) {
      // 对重复提醒的开关，给表单加rules，还有红色星号
      if (val === true) {
        ...
        this.forbidden = true;
        this.redStar = false;
        this.$set(this.rules, "starDtae", [{ type: 'date', required: false, trigger: 'change' }]);
        this.$set(this.rules, "endDtae", [{ type: 'date', required: false, trigger: 'change' }]);
